...
# Last Change: 2024-10-10 06:53:39 d3g096
# -------------------------------------------------------------
...
# download_wmd
# -------------------------------------------------------------
def download_wmd(now, code, thefld, outname, defvalue):

    urlbase = "https://www.nwd-wc.usace.army.mil/dd/common/web_service/webexec/ecsv"
    ...
    for d in daysback:
        ...
        req = urllib2.Request(url, headers=header, context=ctx)
...
def download_wmd_old(now, code, thefld, outname):
    ...
    urlbase = "http://www.nwd-wc.usace.army.mil/dd/common/web_service/webexec/ecsv"
    ...
    if (thefld.lower() == "q"):
        query = "%s.Flow-Out.Ave.1Hour.1Hour.CBT-REV:units=kcfs" % (code)
        offset = timedelta(minutes=30)
        scale = 1000.0
    elif (thefld.lower() == "fb"):
        query = "%s.Elev-Forebay.Inst.1Hour.0.CBT-REV:units=ft" % (code)
    elif (thefld.lower() == "tw"):
        query = "%s.Elev-Tailwater.Inst.1Hour.0.CBT-REV:units=ft" % (code)

    qdata = { "id" : query,
              "startdate" : start.strftime("%m/%d/%Y+08:00"),
              "enddate" : end.strftime("%m/%d/%Y+08:00"),
              "headers" : "true",
              "filename" : "",
              "timezone" : "PST" }
    ...
    for l in f:
        ...
        if (found):
            ...
            # reformat the date
            dstr = fld[0]
            lt = strptime(dstr.lower(), "%d-%b-%Y %H:%M")
...
def download_prdq_current(lastdate, outfile):
    urlbase = "http://grantpud.org/data/water/WQMInfox.php"
    urlbase = "https://www.grantpud.org/WQMInfox.php"
...
def download_prdq_recent(now, outname):
    urlbase = "https://www.grantpud.org/data/water-quality/tdg-supersaturation/hourly/%Y/%m%d%y.csv"
    m = now.month
    d = now.day
    y = now.year
...
try:
    if (dodownload):
        ...
        download_wmd(now, "IHR", "q", "Snake-Flow.dat", 15000.0)
        download_wmd(now, "MCN", "fb", "MCN-FBE.dat", 340.0)
        if (delta.days > 90):
