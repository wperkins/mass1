...
# Last Change: 2018-02-08 13:48:42 d3g096
# -------------------------------------------------------------
...
for code in $fblist; do
    ...
    sed -e "s;@SIMFILE@;$dir/ts${loc}.out;g" \
        -e "s;@OBSFILE@;$bcdir/Flow/${code}-Qtotal.dat;g" \
        -e "s;@OUTPS@;$dir/$code-series-QTL.eps;g" \
        -e "s;@DATA@;Discharge, cfs;g" \
        -e "s;@TAG@;$code QTL;g" \
        -e "s;@NAME@;$mname;g" \
        -e "s;@FLD@;4;g" plot.gp  > tmp.gp
    gnuplot tmp.gp
    
    sed -e "s;@SIMFILE@;$dir/ts${loc}.out;g" \
        -e "s;@OBSFILE@;$bcdir/Stage/${code}-FBE.dat;g" \
        -e "s;@OUTPS@;$dir/$code-series-FBE.eps;g" \
        -e "s;@DATA@;Forebay Stage, feet;g" \
        -e "s;@TAG@;$code QTL;g" \
        -e "s;@NAME@;$mname;g" \
        -e "s;@FLD@;3;g" plot.gp  > tmp.gp
    gnuplot tmp.gp
