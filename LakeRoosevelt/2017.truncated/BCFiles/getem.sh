...
# Daily data only for Banks Canal
# This is actually the reverse: negative should be withdrawl via pump to Banks Lake.  
$massbc -g -Q -O "12 hour" -o BanksCanal-Discharge.dat 12435500 "$start" "$end"
awk -f - BanksCanal-Discharge.dat > BanksCanal-flow.dat <<EOF
NR == 1 { print; next; }
{ printf("%s %s %.3f /\n", \$1, \$2, -\$3); }
EOF
...
$massbc -q -T -0 -o CIBW-Temperature.dat CIBW "$start" "$end"
