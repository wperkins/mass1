...
# Last Change: 2018-03-06 09:08:59 d3g096
# -------------------------------------------------------------
...
while read code loc criteria title; do
    ...
    sed -e "s/@LOC@/$code/g" -e "s/@TITLE@/$title/g" \
        -e "s,@IMPOUNDED@,../../../LowerSnakeColumbia/Temperature,g" \
        compare.gp | gnuplot > $code-temp.compare.eps
done
