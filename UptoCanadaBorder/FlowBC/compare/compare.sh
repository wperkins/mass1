...
fmsname() {
    case $1 in
        ...
        GCGW) name="Grand Coulee" ;;
        CHQW) name="Chief Joseph" ;;
        WELW) name="Wells" ;;
        RRDW) name="Rocky Reach" ;;
        RIGW) name="Rock Island" ;;
        WANW) name="Wanapum" ;;
...
fblist="GCL CHJ WEL RRH MCN RIS WAN PRD"
fmslist="GCGW CHQW RIGW WANW PRXW WELW RRDW"
...
dodisch="yes"                    # set to "yes" for discharge/stage stats/plots
dotemp="no"                    # set to "yes" for temperature stats/plots
...
if [ x"$dodisch" == xyes ]; then
    for code in $fblist; do
        ...
        sed -e "s;@SIMFILE@;$dir/ts${loc}.out;g" \
            -e "s;@OBSFILE@;$bcdir/hydro/stage/${code}_FBE.dat;g" \
            -e "s;@VAR@;wselev;g" \
        ...
        sed -e "s;@SIMFILE@;$dir/ts${loc}.out;g" \
            -e "s;@OBSFILE@;$bcdir/hydro/flow/${code}_flow.dat;g" \
            -e "s;@VAR@;discharge;g" \
            -e "s;@STATSFILE@;$statfile;g" \
